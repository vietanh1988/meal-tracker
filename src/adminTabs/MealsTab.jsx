...
{mob&&<style>{`.mob-qty::placeholder{font-size:11px!important;font-weight:500!important;color:#CBD5E1!important}`}</style>}
...
<input type="text" inputMode="numeric" className="mob-qty" value={isWeight?(item.gram===0?"0":(item.gram??"")):(item.qty??"")} readOnly={!!appliedTemplate} onChange={e=>{if(appliedTemplate)return;const raw=e.target.value.replace(/[^0-9]/g,"");const u={...allFoodItems};const a=[...(u[meal.id]||[])];if(isWeight){a[i]={...a[i],gram:raw===""?"":Number(raw)};}else{const q=raw===""?"":Number(raw);a[i]={...a[i],qty:q,gram:q===""?0:estimateGram(item.name,item.unit,q)};}u[meal.id]=a;setAllFoodItems(u);setUserHasEdited(true);}} style={{width:58,border:"none",outline:"none",background:"transparent",textAlign:"center",fontSize:14,fontWeight:700,color:C.t1,fontFamily:"inherit",padding:"0 2px",opacity:appliedTemplate?0.5:1}} placeholder={isWeight?"VD: 200":"VD: 1"}/>
...
<input type="text" inputMode="numeric" value={item.qty??""} readOnly={!!appliedTemplate} onChange={e=>{if(appliedTemplate)return;const raw=e.target.value.replace(/[^0-9]/g,"");const q=raw===""?"":Number(raw);const u={...allFoodItems};const a=[...(u[meal.id]||[])];a[i]={...a[i],qty:q};if(!isWeight&&q!==""&&q>0){a[i].gram=estimateGram(item.name,item.unit,q);}u[meal.id]=a;setAllFoodItems(u);setUserHasEdited(true);}} style={{...inp,textAlign:"center",fontSize:14,height:40,padding:"10px 12px",opacity:appliedTemplate?0.6:1}} placeholder="SL"/>
<input type="text" inputMode="numeric" value={item.gram===0?"0":(item.gram??"")} readOnly={!!appliedTemplate} onChange={e=>{if(appliedTemplate)return;const raw=e.target.value.replace(/[^0-9]/g,"");const u={...allFoodItems};const a=[...(u[meal.id]||[])];a[i]={...a[i],gram:raw===""?"":Number(raw)};u[meal.id]=a;setAllFoodItems(u);setUserHasEdited(true);}} style={{...inp,textAlign:"center",fontSize:14,height:40,padding:"10px 12px",opacity:appliedTemplate?0.5:(isWeight?1:0.7)}} placeholder={isWeight?"Gram":"~Gram"}/>
...
{!mob&&aiResult&&(()=>{const items=aiResult.items||[];const sl=items.filter(x=>x._mealId===meal.id);if(sl.length===0)return null;const ms=sl.reduce((a,x)=>({p:a.p+(x.protein||0),c:a.c+(x.carb||0),f:a.f+(x.fat||0),fi:a.fi+(x.fiber||0),cal:a.cal+(x.cal||0)}),{p:0,c:0,f:0,fi:0,cal:0});const srcLabel=s=>s==="localDB"?"Kho nội bộ":s==="USDA"?"USDA":s==="cache"?"Cache":s;const srcColor=s=>s==="localDB"?{bg:"#DCFCE7",fg:"#007AFF"}:s==="USDA"?{bg:"#EFF6FF",fg:"#1E40AF"}:s==="cache"?{bg:"#F3F4F6",fg:"#666"}:{bg:"#FEF3C7",fg:"#92400E"};const srcs=[...new Set(sl.map(x=>x.source).filter(Boolean))];return <><div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap",alignItems:"center"}}><span style={{fontSize:11,color:C.t3}}>Nguồn:</span>{srcs.map(s=>{const c=srcColor(s);return <span key={s} style={{fontSize:10,padding:"2px 7px",borderRadius:4,fontWeight:700,background:c.bg,color:c.fg}}>{srcLabel(s)}</span>;})}</div><div style={{display:"flex",gap:14,marginTop:8,paddingTop:8,borderTop:`1px solid ${C.surface}`,flexWrap:"wrap"}}><div style={{fontSize:12,color:C.t2,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.protein}}/> Đạm: <b style={{color:C.t1}}>{Math.round(ms.p)}g</b></div><div style={{fontSize:12,color:C.t2,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.carb}}/> Tinh bột: <b style={{color:C.t1}}>{Math.round(ms.c)}g</b></div><div style={{fontSize:12,color:C.t2,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.fat}}/> Chất béo: <b style={{color:C.t1}}>{Math.round(ms.f)}g</b></div><div style={{fontSize:12,color:C.t2,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.fiber}}/> Chất xơ: <b style={{color:C.t1}}>{Math.round(ms.fi)}g</b></div><div style={{fontSize:12,fontWeight:700,color:C.t1,marginLeft:"auto"}}>{Math.round(ms.cal)} kcal</div></div></>;})()}
</div>;
})}
<button onClick={()=>{
...
return summaryItems?(()=>{const s=summaryItems.reduce((a,i)=>({p:a.p+(i.protein||0),c:a.c+(i.carb||0),f:a.f+(i.fat||0),fi:a.fi+(i.fiber||0),cal:a.cal+(i.cal||0)}),{p:0,c:0,f:0,fi:0,cal:0});const heroCal=dayType==="train"?(macro.calTarget||0):(macro.calRest||macro.calTarget||0);const heroP=macro.protein||0;const heroC=dayType==="train"?(macro.carb||0):(macro.carbRest||macro.carb||0);const heroF=macro.fat||0;const heroFi=macro.fiber||0;const pct=heroCal>0?Math.min(Math.round(s.cal/heroCal*100),120):0;return <><div style={{textAlign:"center",marginBottom:18,paddingBottom:16,borderBottom:`1px solid ${C.surface}`}}><div style={{fontSize:36,fontWeight:800,color:C.primary}}>{Math.round(s.cal).toLocaleString()}</div><div style={{fontSize:14,color:C.t2}}>/ <b style={{color:C.t1}}>{heroCal}</b> kcal mục tiêu</div><div style={{height:8,background:C.surface,borderRadius:4,overflow:"hidden",marginTop:10}}><div style={{height:"100%",borderRadius:4,width:`${Math.min(pct,100)}%`,background:`linear-gradient(90deg,${pct<95?"#F59E0B":pct<=105?"#16A34A":"#DC2626"},${pct<95?"#B45309":pct<=105?"#34C759":"#EF4444"})`}}/></div><div style={{display:"flex",justifyContent:"space-between",marginTop:6}}><span style={{fontSize:11,fontWeight:700,color:pct<95?"#B45309":pct<=105?"#16A34A":"#DC2626"}}>{pct<95?`⚠️ Còn thiếu ${heroCal-Math.round(s.cal)} kcal`:pct<=105?"✅ Ổn rồi, giữ nhé!":`🔴 Dư ${Math.round(s.cal)-heroCal} kcal`}</span><span style={{fontSize:11,color:C.t2}}>{pct}%</span></div></div>{[{l:"Đạm",v:Math.round(s.p),t:heroP,c:C.protein},{l:"Tinh bột",v:Math.round(s.c),t:heroC,c:C.carb},{l:"Chất béo",v:Math.round(s.f),t:heroF,c:C.fat},{l:"Chất xơ",v:Math.round(s.fi),t:heroFi,c:C.fiber}].map(r=><div key={r.l} style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}><div style={{fontSize:13,fontWeight:600,width:70,display:"flex",alignItems:"center",gap:6}}><span style={{width:10,height:10,borderRadius:"50%",background:r.c,flexShrink:0}}/>{r.l}</div><div style={{flex:1,height:6,background:C.surface,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",borderRadius:3,width:`${Math.min(r.t>0?r.v/r.t*100:0,100)}%`,background:r.c}}/></div><div style={{fontSize:12,fontWeight:700,width:80,textAlign:"right"}}>{r.v}g <span style={{fontWeight:400,color:C.t2}}>/ {r.t}g</span></div></div>)}</>;})():<div style={{textAlign:"center",padding:"40px 20px",color:C.t3}}><div style={{fontSize:36,marginBottom:8}}>📊</div><div style={{fontSize:14,fontWeight:600,color:C.t2}}>Chưa có dữ liệu</div><div style={{fontSize:12,color:C.t3,marginTop:4}}>Nhấn "Tính macro tất cả" để xem kết quả</div></div>;
})()}
</div>
{(()=>{
