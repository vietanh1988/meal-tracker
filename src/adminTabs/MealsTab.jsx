...
return mob ? (
<div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"12px 0",borderBottom:i<foods.length-1?`1px solid ${C.border}`:"none"}}>
<span style={{fontSize:12,fontWeight:700,color:C.t3,width:16,textAlign:"center",flexShrink:0}}>{i+1}</span>
<input value={item.name} readOnly={!!appliedTemplate} onChange={e=>{if(appliedTemplate)return;const u={...allFoodItems};const a=[...(u[meal.id]||[])];a[i]={...a[i],name:e.target.value};u[meal.id]=a;setAllFoodItems(u);setUserHasEdited(true);}} placeholder="VD: Cá kho" style={{flex:1,minWidth:0,border:"none",outline:"none",fontSize:15,fontWeight:700,color:C.t1,fontFamily:"inherit",background:"transparent",padding:0,opacity:appliedTemplate?0.6:1}}/>
<div style={{display:"flex",alignItems:"center",background:C.surface,borderRadius:8,border:`1px solid ${C.border}`,overflow:"hidden",height:38,flexShrink:0}}>
...
<span onClick={()=>{if(appliedTemplate)return;const u={...allFoodItems};const a=[...(u[meal.id]||[])];a.splice(i,1);if(a.length===0)a.push({name:"",gram:"",unit:"g",qty:1});u[meal.id]=a;setAllFoodItems(u);setUserHasEdited(true);}} style={{fontSize:18,color:C.t3,cursor:appliedTemplate?"not-allowed":"pointer",opacity:appliedTemplate?0.4:0.5,flexShrink:0,padding:"0 2px"}}>×</span>
</div>
) : (
