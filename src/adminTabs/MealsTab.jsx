...
<input type="number" {...numFix} inputMode="numeric" value={isWeight?(item.gram===0?0:(item.gram||"")):(item.qty||"")} readOnly={!!appliedTemplate} onChange={e=>{if(appliedTemplate)return;const val=Math.max(0,Number(e.target.value)||0);const u={...allFoodItems};const a=[...(u[meal.id]||[])];if(isWeight){a[i]={...a[i],gram:val};}else{a[i]={...a[i],qty:val,gram:estimateGram(item.name,item.unit,val)};}u[meal.id]=a;setAllFoodItems(u);setUserHasEdited(true);}} style={{width:50,border:"none",outline:"none",background:"transparent",textAlign:"center",fontSize:15,fontWeight:700,color:C.t1,fontFamily:"inherit",padding:"0 4px",opacity:appliedTemplate?0.5:1}} placeholder={isWeight?"g":"1"}/>
