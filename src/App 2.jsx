...
{"items":[{"name":"tên","gram":số,"protein":số,"carb":số,"fat":số,"fiber":số,"cal":số}],"tip":"1 câu gợi ý cho người gym tăng cơ"}`;
  ...
  const callAI=useCallback(async()=>{
    ...
    const foodDesc=foodItems.filter(f=>f.name.trim()).map(f=>`${f.qty>1?f.qty+" ":""}${f.name} ${f.gram}g`).join(", ");
    try{
      let text="";
  ...
  return <div>
    ...
    {section==="ai"&&<div style={card}>
      ...
      {aiProvider==="gpt"&&<>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
          {[
            {id:"gpt-5.4-mini",name:"GPT-5.4 Mini",desc:"Nhanh, rẻ nhất",badge:"Tiết kiệm",bc:C.greenBg,btc:"#14532D"},
    ...
    {section==="weight"&&(()=>{
      ...
      return <div style={card}>
        ...
        <div style={{borderTop:`1.5px solid ${C.border}`,paddingTop:14,marginTop:16}}>
          <div style={{...lbl,marginBottom:10}}>Lịch sử theo dõi cân nặng</div>
          <div style={{display:"grid",gridTemplateColumns:"0.8fr 1.2fr 1fr 0.8fr",gap:4,fontSize:11,fontWeight:700,color:C.t3,paddingBottom:6,marginBottom:4,borderBottom:`1px solid ${C.border}`,textTransform:"uppercase",letterSpacing:"0.05em"}}>
            <span>Tuần</span><span>Ngày</span><span style={{textAlign:"right"}}>Cân nặng</span><span style={{textAlign:"right"}}>Thay đổi</span>
          </div>
          {weightLog.map((w,i)=><div key={i} style={{display:"grid",gridTemplateColumns:"0.8fr 1.2fr 1fr 0.8fr",gap:4,padding:"10px 0",fontSize:14,borderBottom:i<weightLog.length-1?`1px solid ${C.border}`:"none"}}>
            <span style={{fontWeight:800,color:wColors[i%wColors.length]}}>Tuần {w.week}</span>
            <span style={{fontWeight:600,color:C.t2}}>{w.date}</span>
            <span style={{color:C.t1,fontWeight:900,textAlign:"right"}}>{w.kg} kg</span>
            <span style={{color:w.delta?w.delta>0?C.green:C.red:C.t3,fontWeight:800,fontSize:13,textAlign:"right"}}>{w.delta?`${w.delta>0?"+":""}${w.delta} kg`:"—"}</span>
          </div>)}
        </div>
        {weightLog.length>=2&&(()=>{
...
export default function App(){
  ...
  const {weightLog,addWeight,loading:weightLoading}=useWeightLog(user?.id);
  const macro=profile?calcMacro(profile):calcMacro({cm:172,kg:63,age:25,goalKg:68,gym:4,goalType:"bulk",months:4,activity:"sedentary"});
  const mob=useIsMobile();
  ...
  return <div style={{fontFamily:"'Inter',-apple-system,sans-serif",background:C.bg,color:C.t1,minHeight:"100vh",padding:mob?"12px":"16px 20px",maxWidth:700,margin:"0 auto"}}>
    ...
    {tab==="dashboard"?<Dashboard weightLog={weightLog} profile={profile} macro={macro}/>:<AdminPanel weightLog={weightLog} setWeightLog={addWeight} profile={profile} setProfile={setProfile} macro={macro}/>}
  </div>;
}
