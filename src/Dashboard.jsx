...
export function Dashboard({weightLog,addWeight,profile,setProfile,macro,getMeals,getTodayMeals,hasMealsToday,appSettings,setTab,user,getWeeklyTemplate,applyTemplate,saveWeeklyTemplate,getMealHistory,getDailyLogs,userDataLoaded,macroBanner,eatenMeals,toggleEaten,onOpenPhotoMacro}){if(!profile||!macro)return null;
  ...
  // Sync dayType — gymDays quyết định, không nhảy bucket theo data
  useEffect(()=>{
    ...
    {(() => {
      ...
      return <>
        ...
        {allEaten && <div style={{textAlign:"center",padding:"20px 16px",background:"linear-gradient(135deg,#FFF7ED,#FEF3C7,#FDE68A)",borderRadius:16,border:"1.5px solid #F59E0B",marginBottom:16,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{fontSize:16,fontWeight:900,color:"#92400E",position:"relative"}}>Đã hoàn thành {eatenCount}/{mealsWithItems.length} bữa ăn!</div>
          <div style={{fontSize:12,color:"#B45309",marginTop:4,fontWeight:600,position:"relative"}}>{actualCal.toLocaleString()} / {heroCal.toLocaleString()} kcal · {calOk?"đạt mục tiêu":"còn thiếu "+Math.max(0,calShort)+" kcal"}</div>
          {calOk&&<div style={{display:"inline-flex",alignItems:"center",gap:4,marginTop:8,padding:"4px 10px",background:"rgba(245,158,11,.15)",borderRadius:6,position:"relative"}}>
      ...
      return <div style={{...card,padding:"14px 16px",marginTop:6,background:"rgba(52,199,89,0.04)",border:"1.5px solid rgba(52,199,89,0.15)"}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div style={{flex:1,borderLeft:"1.5px solid rgba(52,199,89,0.15)",paddingLeft:14}}><div style={{fontSize:13,fontWeight:700,color:C.t1}}>{msl}</div><div style={{fontSize:12,color:C.t2,marginTop:3,lineHeight:1.5}}>{(()=>{
            ...
            const uneatenMeals=mealsAll.filter(m=>!(eatenMeals||[]).includes(m.id));
