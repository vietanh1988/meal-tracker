...
export default function App(){
  ...
  if(mob) return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",background:C.bg,color:C.t1,minHeight:"100vh",padding:"0 10px 10px 10px",maxWidth:700,margin:"0 auto",overflowX:"hidden",width:"100%",boxSizing:"border-box"}}>
    ...
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:99,background:"rgba(255,255,255,0.97)",borderTop:"0.5px solid rgba(0,0,0,0.12)",display:"flex",alignItems:"flex-end",paddingBottom:"max(12px, env(safe-area-inset-bottom, 12px))"}}>
      ...
      {[{id:"weight",label:"Cân nặng",svg:(c)=><svg viewBox="0 0 96 96" width={24} height={24}><rect x="8" y="78" width="80" height="10" rx="5" fill={c}/><rect x="44" y="28" width="8" height="52" rx="4" fill={c}/><rect x="12" y="24" width="72" height="8" rx="4" fill={c}/><rect x="22" y="24" width="4" height="18" rx="2" fill={c}/><rect x="70" y="24" width="4" height="18" rx="2" fill={c}/><rect x="10" y="40" width="28" height="8" rx="4" fill={c}/><rect x="58" y="40" width="28" height="8" rx="4" fill={c}/><circle cx="48" cy="16" r="8" fill={c}/></svg>},{id:"diary",label:"Nhật ký",svg:(c)=><svg viewBox="0 0 96 96" width={24} height={24}><rect x="18" y="8" width="60" height="80" rx="8" fill={c}/><rect x="14" y="20" width="8" height="4" rx="2" fill={c}/><rect x="14" y="32" width="8" height="4" rx="2" fill={c}/><rect x="14" y="44" width="8" height="4" rx="2" fill={c}/><rect x="14" y="56" width="8" height="4" rx="2" fill={c}/><rect x="14" y="68" width="8" height="4" rx="2" fill={c}/><rect x="30" y="24" width="36" height="5" rx="2.5" fill="white" opacity="0.8"/><rect x="30" y="36" width="28" height="5" rx="2.5" fill="white" opacity="0.5"/><rect x="30" y="48" width="32" height="5" rx="2.5" fill="white" opacity="0.5"/><rect x="30" y="60" width="24" height="5" rx="2.5" fill="white" opacity="0.35"/><polyline points="56,70 60,74 68,64" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>}].map(t=>{const a=tab===t.id;return <div key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:1,cursor:"pointer",padding:"7px 0 0"}}>{t.svg(a?"url(#navG)":"#A0A0A0")}<span style={{fontSize:9,fontWeight:a?700:500,color:a?"#007AFF":"#8E8E93"}}>{t.label}</span></div>;})}
    </div>
    </div>
    <PWAInstallPrompt />
  ...
  return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",display:"flex",minHeight:"100vh",background:C.bg,color:C.t1}}>
    ...
    <nav style={{width:220,background:"#fff",borderRight:`1px solid ${C.border}`,position:"fixed",top:0,left:0,bottom:0,zIndex:10,display:"flex",flexDirection:"column",padding:"20px 0",overflowY:"auto"}}>
      ...
      {[{id:"dashboard",l:"Tổng quan",ic:"dashboard"},{id:"meals",l:"Bữa ăn",ic:"meals"},{id:"weight",l:"Cân nặng",ic:"weight"},{id:"diary_s",l:"Nhật ký",ic:"diary"}].map(s=><div key={s.id} onClick={()=>setTab(s.id)} style={{display:"flex",alignItems:"center",gap:11,padding:"9px 20px",cursor:"pointer",fontSize:14,fontWeight:tab===s.id?700:500,color:tab===s.id?C.primary:C.t2,background:tab===s.id?"rgba(0,122,255,0.06)":"transparent",borderLeft:tab===s.id?`3px solid ${C.primary}`:"3px solid transparent"}}>{pcNavI(s.ic,tab===s.id,19)} {s.l}</div>)}
      {(()=>{
        const items=[
          {id:"report",l:"Báo cáo",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><defs><linearGradient id="sir" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="100%" stopColor={a?"#0050FF":"#64748B"}/></linearGradient></defs><rect x="8" y="56" width="22" height="32" rx="5" fill="url(#sir)"/><rect x="37" y="36" width="22" height="52" rx="5" fill="url(#sir)"/><rect x="66" y="16" width="22" height="72" rx="5" fill="url(#sir)"/><rect x="4" y="90" width="88" height="6" rx="3" fill="url(#sir)"/></svg>},
      ...
      <main style={{padding:tab==="account"?"92px 100px 24px":"92px 24px 24px",flex:1}}>
        {tab==="dashboard"&&<div>
          ...
          {pcAC>0&&pcCR>0&&<div style={{...card,padding:"18px 24px",marginTop:20,display:"flex",alignItems:"center",gap:16}}>
            ...
            <div style={{minWidth:0}}><div style={{fontSize:14,fontWeight:700,color:C.t1}}>Fipilot AI gợi ý cho bạn</div><div style={{fontSize:12,color:C.t2,marginTop:3}}>Hôm nay bạn đang thiếu <b style={{color:"#D97706"}}>{Math.round(Math.max(0,pcHC-pcACb))}g tinh bột</b> để đạt mục tiêu.</div></div>
