...
export default function App(){
  ...
  const [adminGrpOpen,setAdminGrpOpen]=useState({content:true,users:true});
  ...
  const adminP={weightLog,setWeightLog,addWeight,deleteWeight,resetWeights,profile,setProfile:wrappedSetProfile,macro,saveMealToCloud,saveFoodCache,deleteFoodCache,getMeals,foodCache,appSettings,isAdmin,saveSetting,weeklyTemplates,saveWeeklyTemplate,getWeeklyTemplate,defaultTemplates,saveDefaultTemplate,deleteDefaultTemplate,applyTemplate,refreshDefaultTemplates,user};
      ...
      {isAdmin&&(()=>{
        const groups=[
          ...
          {key:"users",label:"Người dùng",items:[
            {id:"users_s",l:"Quản lý User",svg:(a)=><svg viewBox="0 0 96 96" width={20} height={20}><defs><linearGradient id="qi4" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><circle cx="34" cy="34" r="18" fill="url(#qi4)"/><path d="M8 88 C8 58 60 58 60 88 Z" fill="url(#qi4)"/><circle cx="70" cy="40" r="13" fill="url(#qi4)" opacity="0.6"/><path d="M52 88 C52 66 88 66 88 88 Z" fill="url(#qi4)" opacity="0.6"/></svg>},
            {id:"orders_s",l:"Duyệt đơn hàng",svg:(a)=><svg viewBox="0 0 96 96" width={20} height={20}><defs><linearGradient id="qi5" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><rect x="18" y="10" width="60" height="76" rx="10" fill="url(#qi5)"/><rect x="28" y="26" width="40" height="6" rx="3" fill="white" opacity="0.8"/><rect x="28" y="40" width="40" height="6" rx="3" fill="white" opacity="0.5"/><circle cx="62" cy="66" r="16" fill="white"/><path d="M55 66 L60 71 L70 60" fill="none" stroke="url(#qi5)" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"/></svg>},
            {id:"subscription_settings_s",l:"Gói cước",svg:(a)=><svg viewBox="0 0 96 96" width={20} height={20}><defs><linearGradient id="qi3" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><circle cx="48" cy="48" r="42" fill="url(#qi3)"/><path d="M48 22 L54 42 L74 42 L58 54 L64 74 L48 62 L32 74 L38 54 L22 42 L42 42 Z" fill="white" opacity="0.9"/></svg>},
            {id:"report_biz_s",l:"Báo cáo kinh doanh",svg:(a)=><svg viewBox="0 0 96 96" width={20} height={20}><defs><linearGradient id="qi6" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><rect x="10" y="10" width="76" height="76" rx="12" fill="url(#qi6)"/><rect x="24" y="52" width="10" height="24" rx="3" fill="white"/><rect x="43" y="38" width="10" height="38" rx="3" fill="white" opacity="0.85"/><rect x="62" y="26" width="10" height="50" rx="3" fill="white" opacity="0.7"/></svg>},
          ]},
        ];
        return <>
          <div style={{fontSize:10,fontWeight:700,color:"#EF4444",padding:"0 20px",margin:"16px 0 6px",letterSpacing:"0.8px"}}>QUẢN TRỊ</div>
          {groups.map(g=>{
            const open=adminGrpOpen[g.key];
            const hasActive=g.items.some(it=>it.id===tab);
            return <div key={g.key}>
              <div onClick={()=>setAdminGrpOpen(p=>({...p,[g.key]:!p[g.key]}))} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 20px",cursor:"pointer",fontSize:13,fontWeight:700,color:hasActive?C.primary:C.t1,userSelect:"none"}}>
                <span style={{flex:1}}>{g.label}</span>
                <svg width={12} height={12} viewBox="0 0 24 24" style={{transform:open?"rotate(0deg)":"rotate(-90deg)",transition:"transform .15s"}}><path d="M6 9l6 6 6-6" stroke={C.t2} strokeWidth={2.5} fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </div>
              {open&&g.items.map(s=>{const a=tab===s.id;return <div key={s.id} onClick={()=>setTab(s.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"9px 20px 9px 34px",cursor:"pointer",fontSize:13,fontWeight:a?700:500,color:a?C.primary:C.t2,background:a?"rgba(0,122,255,0.06)":"transparent",borderLeft:a?`3px solid ${C.primary}`:"3px solid transparent"}}>{s.svg(a)} {s.l}</div>;})}
            </div>;
          })}
        </>;
      })()}
