  ...
  return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",display:"flex",minHeight:"100vh",background:C.bg,color:C.t1}}>
    ...
    <div style={{marginLeft:220,flex:1,display:"flex",flexDirection:"column"}}>
      <header style={{height:68,display:"flex",alignItems:"center",padding:"0 28px",background:"#fff",borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:5}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{display:"flex",borderRadius:10,overflow:"hidden",border:`1.5px solid ${C.border}`}}><div onClick={()=>setPcDayType("train")} style={{padding:"7px 16px",fontSize:12,fontWeight:700,cursor:"pointer",background:pcDayType==="train"?C.primary:"#fff",color:pcDayType==="train"?"#fff":C.t2}}>🏋️ Ngày tập</div><div onClick={()=>setPcDayType("rest")} style={{padding:"7px 16px",fontSize:12,fontWeight:700,cursor:"pointer",background:pcDayType==="rest"?C.primary:"#fff",color:pcDayType==="rest"?"#fff":C.t2}}>😴 Ngày nghỉ</div></div>
          <NotiBell appSettings={appSettings}/>
      ...
      <main style={{padding:24,flex:1}}>
        {tab==="dashboard"&&<div>
          ...
          <div style={{...card,padding:"28px 32px",borderRadius:20,display:"flex",alignItems:"center",marginBottom:24,border:`1.5px solid ${C.border}`}}>
            <div style={{flex:"0 0 45%"}}><div style={{fontSize:11,fontWeight:700,color:"#64748B",letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:8}}>{pcDayType==="train"?"Tổng calo ngày tập":"Tổng calo ngày nghỉ"}</div><div style={{fontSize:48,fontWeight:900,color:C.t1,letterSpacing:"-2px",lineHeight:1}}>{pcAC>0?pcAC.toLocaleString():pcHCal.toLocaleString()} <span style={{fontSize:17,fontWeight:600,color:"#64748B"}}> / {pcHCal.toLocaleString()} kcal</span></div>{pcAC>0&&<div style={{marginTop:10,fontSize:13,fontWeight:700,color:pcAC>=pcHCal*0.95&&pcAC<=pcHCal*1.1?"#34C759":"#F59E0B"}}>{pcAC>=pcHCal*0.95&&pcAC<=pcHCal*1.1?`✅ Đã đạt mục tiêu! ${pcAC-pcHCal} kcal`:`⚠️ Còn thiếu ${pcCR} kcal`}</div>}<div style={{display:"flex",alignItems:"center",gap:10,marginTop:14,maxWidth:320}}><div style={{flex:1,height:6,background:C.border,borderRadius:3}}><div style={{height:6,background:"linear-gradient(90deg,#36A3FF,#007AFF)",borderRadius:3,width:`${Math.min(pcAC>0?(pcAC/pcHCal)*100:0,120)}%`,transition:"width 0.4s"}}/></div><span style={{fontSize:13,fontWeight:700,color:C.primary}}>{pcAC>0?Math.round(pcAC/pcHCal*100):0}%</span></div></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"55fr 45fr",gap:24}}>
            ...
            <div>
              <div style={{...card,padding:18,marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><span style={{fontSize:15,fontWeight:800,color:C.t1}}>Theo dõi cân nặng</span><span style={{fontSize:11,color:C.t2}}>🎯 {pcGK} kg</span></div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:10}}>{[{l:"Xuất phát",v:pcSK,c:C.t1},{l:"Hiện tại",v:pcCK,c:C.primary},{l:"Mục tiêu",v:pcGK,c:C.t1},{l:"Tiến độ",v:Math.round(Math.max(0,Math.min(pcWP,100)))+"%",c:C.primary}].map((w,i)=><div key={i} style={{textAlign:"center",padding:"8px 4px",background:C.surface,borderRadius:8}}><div style={{fontSize:9,color:"#64748B",fontWeight:600}}>{w.l}</div><div style={{fontSize:18,fontWeight:800,color:w.c}}>{w.v}</div>{typeof w.v==="number"&&<div style={{fontSize:9,color:"#64748B"}}>kg</div>}</div>)}</div>
                <div style={{fontSize:11,fontWeight:600,display:"flex",justifyContent:"space-between",marginBottom:5}}><span>Đã {pcCK>=pcSK?"tăng":"giảm"} {Math.abs(Math.round((pcCK-pcSK)*10)/10)} kg</span><span style={{color:C.primary}}>{Math.round(Math.max(0,Math.min(pcWP,100)))}%</span></div>
                <div style={{height:5,background:C.surface,borderRadius:3}}><div style={{height:5,borderRadius:3,background:"linear-gradient(90deg,#36A3FF,#007AFF)",width:`${Math.max(0,Math.min(pcWP,100))}%`}}/></div>
