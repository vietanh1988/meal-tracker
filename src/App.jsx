...
export default function App(){
  ...
  if(mob) return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",background:C.bg,color:C.t1,minHeight:"100vh",padding:"0 10px 10px 10px",maxWidth:700,margin:"0 auto",overflowX:"hidden",width:"100%",boxSizing:"border-box"}}>
    ...
    {/* AI Coach FAB */}
    {!showAICoach&&<div onClick={()=>setShowAICoach(true)} style={{position:"fixed",bottom:78,right:16,width:52,height:52,borderRadius:"50%",background:"linear-gradient(135deg,#36A3FF,#007AFF)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,color:"#fff",boxShadow:"0 4px 14px rgba(0,122,255,0.4)",zIndex:98,cursor:"pointer"}}>✨</div>}
    {showAICoach&&<AICoachPanel profile={profile} macro={macro} weightLog={weightLog} todayData={{cal:0,p:0,c:0,f:0,dayType:(profile?.gymDays||[]).includes(new Date().getDay())?"train":"rest"}} mob={true} onClose={()=>setShowAICoach(false)} appSettings={appSettings} isAdmin={isAdmin}/>}
