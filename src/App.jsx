...
export default function App(){
  ...
  const {loaded:userDataLoaded,meals:cloudMeals,getMeals,getTodayMeals,hasMealsToday,getMealHistory,foodCache,saveMealToCloud,saveFoodCache,deleteFoodCache,weeklyTemplates,saveWeeklyTemplate,deleteWeeklyTemplate,getWeeklyTemplate,defaultTemplates,saveDefaultTemplate,deleteDefaultTemplate,refreshDefaultTemplates,weeklyBundles,saveWeeklyBundle,deleteWeeklyBundle,refreshWeeklyBundles,applyTemplate,saveDailyLog,getDailyLogs,getDailyLog,eatenMeals,toggleEaten}=useUserData(user?.id);
  ...
  if(needsOnboarding) return <OnboardingWizard profile={profile} setProfile={wrappedSetProfile} onComplete={()=>setTab("dashboard")} appSettings={appSettings} user={user} saveWeeklyTemplate={saveWeeklyTemplate} applyTemplate={applyTemplate} getMealHistory={getMealHistory}/>;
  ...
  if(mob) return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",background:C.bg,color:C.t1,minHeight:"100vh",padding:"0 10px 10px 10px",maxWidth:700,margin:"0 auto",overflowX:"hidden",width:"100%",boxSizing:"border-box"}}>
    ...
    {tab==="diary"&&<div style={{padding:10}}><DiaryTab userId={user?.id} macro={macro}/></div>}
    ...
    {flags.ai_chat&&!showAICoach&&<div onClick={()=>setShowAICoach(true)} style={{position:"fixed",bottom:100,right:14,width:56,height:56,borderRadius:16,background:"linear-gradient(135deg,#7C3AED,#5B21B6)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#fff",zIndex:98,cursor:"pointer",transition:"opacity 0.25s ease, transform 0.25s ease",opacity:aiFabHidden?0:1,transform:aiFabHidden?"translateY(20px) scale(0.85)":"translateY(0) scale(1)",pointerEvents:aiFabHidden?"none":"auto",animation:aiFabHidden?"none":"fabPurpleGlow 2s ease-in-out infinite"}}><span style={{fontSize:20}}>✨</span><span style={{fontSize:7,fontWeight:800,letterSpacing:"0.3px",opacity:0.9,marginTop:1}}>Fipilot AI</span></div>}
    {flags.ai_chat&&showAICoach&&<AICoachPanel profile={profile} macro={macro} weightLog={weightLog} todayData={mobTodayData} mob={true} onClose={()=>setShowAICoach(false)} appSettings={appSettings} isAdmin={isAdmin} getMeals={getMeals} getWeeklyTemplate={getWeeklyTemplate} foodCache={foodCache} userId={user?.id} applyTemplate={applyTemplate} saveWeeklyTemplate={saveWeeklyTemplate} getMealHistory={getMealHistory} getDailyLogs={getDailyLogs}/>}
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:99,background:"rgba(255,255,255,0.97)",borderTop:"0.5px solid rgba(0,0,0,0.12)",display:"flex",alignItems:"flex-end",paddingBottom:"max(12px, env(safe-area-inset-bottom, 12px))"}}>
      {[{id:"dashboard",label:"Tổng quan",svg:(c)=><svg viewBox="0 0 96 96" width={24} height={24}><rect x="6" y="6" width="38" height="38" rx="10" fill={c}/><rect x="52" y="6" width="38" height="38" rx="10" fill={c}/><rect x="6" y="50" width="38" height="32" rx="10" fill={c}/><rect x="52" y="50" width="38" height="32" rx="10" fill={c}/><rect x="6" y="86" width="84" height="8" rx="4" fill={c}/></svg>},{id:"meals",label:"Bữa ăn",svg:(c)=><svg viewBox="0 0 96 96" width={24} height={24}><rect x="6" y="6" width="84" height="84" rx="14" fill={c}/><circle cx="22" cy="30" r="6" fill="white" opacity="0.9"/><rect x="36" y="25" width="46" height="10" rx="5" fill="white" opacity="0.9"/><circle cx="22" cy="52" r="6" fill="white" opacity="0.9"/><rect x="36" y="47" width="36" height="10" rx="5" fill="white" opacity="0.9"/><circle cx="22" cy="74" r="6" fill="white" opacity="0.9"/><rect x="36" y="69" width="40" height="10" rx="5" fill="white" opacity="0.9"/></svg>}].map(t=>{const a=tab===t.id;return <div key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:1,cursor:"pointer",padding:"7px 0 0"}}>{t.svg(a?"url(#navG)":"#A0A0A0")}<span style={{fontSize:9,fontWeight:a?700:500,color:a?"#007AFF":"#8E8E93"}}>{t.label}</span></div>;})}
      <div onClick={()=>{
        if(!flags.photo_macro){alert("📸 Tính năng Chụp ảnh kiểm tra dinh dưỡng hiện đã bị tạm tắt bởi quản trị viên.");return;}
        setShowPhotoMacro(true);
      }} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",cursor:"pointer",marginTop:-18,opacity:flags.photo_macro?1:0.5}}>
        <div style={{width:48,height:48,borderRadius:"50%",background:flags.photo_macro?"linear-gradient(135deg,#F97316,#EA580C)":"#94A3B8",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:flags.photo_macro?"0 3px 12px rgba(249,115,22,0.45)":"none",border:"3px solid #fff"}}>
          <svg viewBox="0 0 24 24" width={20} height={20} fill="none" stroke="#fff" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>
        </div>
        <span style={{fontSize:8,fontWeight:700,color:flags.photo_macro?"#EA580C":"#94A3B8",marginTop:2}}>Chụp món ăn</span>
      </div>
      {[{id:"weight",label:"Cân nặng",svg:(c)=><svg viewBox="0 0 96 96" width={24} height={24}><rect x="8" y="78" width="80" height="10" rx="5" fill={c}/><rect x="44" y="28" width="8" height="52" rx="4" fill={c}/><rect x="12" y="24" width="72" height="8" rx="4" fill={c}/><rect x="22" y="24" width="4" height="18" rx="2" fill={c}/><rect x="70" y="24" width="4" height="18" rx="2" fill={c}/><rect x="10" y="40" width="28" height="8" rx="4" fill={c}/><rect x="58" y="40" width="28" height="8" rx="4" fill={c}/><circle cx="48" cy="16" r="8" fill={c}/></svg>},{id:"diary",label:"Nhật ký",svg:(c)=><svg viewBox="0 0 96 96" width={24} height={24}><rect x="14" y="6" width="68" height="84" rx="8" fill={c}/><rect x="8" y="20" width="10" height="4" rx="2" fill={c}/><rect x="8" y="32" width="10" height="4" rx="2" fill={c}/><rect x="8" y="44" width="10" height="4" rx="2" fill={c}/><rect x="8" y="56" width="10" height="4" rx="2" fill={c}/><rect x="8" y="68" width="10" height="4" rx="2" fill={c}/><rect x="28" y="22" width="42" height="5" rx="2.5" fill="white" opacity="0.8"/><rect x="28" y="34" width="32" height="5" rx="2.5" fill="white" opacity="0.5"/><rect x="28" y="46" width="36" height="5" rx="2.5" fill="white" opacity="0.5"/><rect x="28" y="58" width="26" height="5" rx="2.5" fill="white" opacity="0.35"/><polyline points="58,68 62,73 72,62" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>}].map(t=>{const a=tab===t.id;return <div key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:1,cursor:"pointer",padding:"7px 0 0"}}>{t.svg(a?"url(#navG)":"#A0A0A0")}<span style={{fontSize:9,fontWeight:a?700:500,color:a?"#007AFF":"#8E8E93"}}>{t.label}</span></div>;})}
    </div>
    </div>
    <PWAInstallPrompt />
    {showPhotoMacro&&<PhotoMacroChecker onClose={()=>setShowPhotoMacro(false)} appSettings={appSettings} hasMealsToday={hasMealsToday} saveMealToCloud={saveMealToCloud} toggleEaten={toggleEaten} getTodayMeals={getTodayMeals} photoCtx={(()=>{try{const gd=profile?.gymDays||[0,2,4,5];const isNone=profile?.exerciseType==="none";const rawDow=new Date().getDay();const todayDow=rawDow===0?6:rawDow-1;const todayIsGym=!isNone&&gd.includes(todayDow);const activeDayType=todayIsGym?"train":"rest";const mc=(()=>{if(profile.mealConfig)return profile.mealConfig;try{return appSettings.meal_config?JSON.parse(appSettings.meal_config):null;}catch(e){return null;}})();const ids=(mc&&mc[activeDayType])||["sang","trua","toi"];const visIds=isNone?ids.filter(id=>id!=="pre"&&id!=="post"):ids;const meals=getTodayMeals(activeDayType);const eatenCal=meals.filter(m=>eatenMeals.includes(m.id)).reduce((a,m)=>a+m.items.reduce((x,i)=>x+(i.cal||0),0),0);return{target:activeDayType==="train"?macro.calTarget:macro.calRest,eatenCal:Math.round(eatenCal),goal:macro.goal,proteinTarget:macro.protein,dayType:activeDayType,visibleMealIds:visIds};}catch(e){return null;}})()}/>}
    ...
    <nav style={{width:220,background:"#fff",borderRight:`1px solid ${C.border}`,position:"fixed",top:0,left:0,bottom:0,zIndex:10,display:"flex",flexDirection:"column",padding:"20px 0",overflowY:"auto"}}>
      ...
      {(()=>{
        const items=[
          ...
          ...(!isAdmin?[{id:"feedback",l:"Góp ý & Phản hồi",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><defs><linearGradient id="sifb" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="100%" stopColor={a?"#0050FF":"#64748B"}/></linearGradient></defs><rect x="6" y="12" width="84" height="60" rx="14" fill="url(#sifb)"/><path d="M24 72 L24 84 L42 72" fill="url(#sifb)"/><circle cx="30" cy="42" r="6" fill="white" opacity="0.9"/><circle cx="48" cy="42" r="6" fill="white" opacity="0.9"/><circle cx="66" cy="42" r="6" fill="white" opacity="0.9"/></svg>}]:[]),
        ];
      ...
      {isAdmin&&(()=>{
        const items=[
          ...
          {id:"weekly_bundles_s",l:"Gói tuần",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><defs><linearGradient id="qi12" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><rect x="10" y="14" width="76" height="72" rx="10" fill="url(#qi12)"/><rect x="10" y="14" width="76" height="18" rx="10" fill="white" opacity="0.15"/><rect x="22" y="4" width="8" height="16" rx="4" fill="url(#qi12)"/><rect x="66" y="4" width="8" height="16" rx="4" fill="url(#qi12)"/><rect x="24" y="46" width="14" height="14" rx="3" fill="white" opacity="0.5"/><rect x="41" y="46" width="14" height="14" rx="3" fill="white" opacity="0.3"/><rect x="58" y="46" width="14" height="14" rx="3" fill="white" opacity="0.5"/><rect x="24" y="64" width="14" height="14" rx="3" fill="white" opacity="0.3"/><rect x="41" y="64" width="14" height="14" rx="3" fill="white" opacity="0.5"/></svg>},
          ...
          {id:"ai_cost_s",l:"Chi phí AI",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><defs><linearGradient id="qi13" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><circle cx="48" cy="48" r="40" fill="url(#qi13)"/><path d="M62 34 C62 26 55 22 46 22 C37 22 30 27 30 33 C30 40 36 43 46 48 C56 53 62 56 62 63 C62 70 55 74 46 74 C37 74 30 70 30 62" fill="none" stroke="white" strokeWidth="8" strokeLinecap="round" opacity="0.9"/><line x1="46" y1="10" x2="46" y2="24" stroke="white" strokeWidth="7" strokeLinecap="round" opacity="0.9"/><line x1="46" y1="72" x2="46" y2="86" stroke="white" strokeWidth="7" strokeLinecap="round" opacity="0.9"/></svg>},
          ...
          {id:"food_cache_pending_s",l:"Kho món",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><defs><linearGradient id="qi14" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><path d="M14 38 L48 22 L82 38 L82 78 C82 82 78 86 74 86 L22 86 C18 86 14 82 14 78 Z" fill="url(#qi14)"/><path d="M14 38 L48 54 L82 38" fill="none" stroke="white" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" opacity="0.4"/><path d="M48 54 L48 86" stroke="white" strokeWidth="5" strokeLinecap="round" opacity="0.3"/></svg>},
          {id:"food_db_s",l:"Kho thực phẩm",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><defs><linearGradient id="qi15" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><rect x="10" y="10" width="76" height="76" rx="14" fill="url(#qi15)"/><ellipse cx="48" cy="30" rx="24" ry="8" fill="white" opacity="0.9"/><path d="M24 30 v12 c0 5 10 8 24 8 s24-3 24-8 v-12" fill="white" opacity="0.5"/><path d="M24 42 v12 c0 5 10 8 24 8 s24-3 24-8 v-12" fill="white" opacity="0.3"/></svg>},
          ...
          {id:"admin_feedback_s",l:"Phản hồi KH",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><defs><linearGradient id="qi8" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><rect x="6" y="12" width="84" height="60" rx="14" fill="url(#qi8)"/><path d="M24 72 L24 84 L42 72" fill="url(#qi8)"/><circle cx="30" cy="42" r="6" fill="white" opacity="0.9"/><circle cx="48" cy="42" r="6" fill="white" opacity="0.9"/><circle cx="66" cy="42" r="6" fill="white" opacity="0.9"/></svg>},
          ...
          {id:"backup_s",l:"Sao lưu",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><rect x="8" y="20" width="80" height="56" rx="12" fill={a?"#3B82F6":"#64748B"}/><path d="M36 48 L48 60 L60 44" fill="none" stroke="white" strokeWidth="7" strokeLinecap="round" strokeLinejoin="round"/><rect x="30" y="8" width="36" height="18" rx="6" fill={a?"#60A5FA":"#94A3B8"}/></svg>},
        ].filter(s=>s.id!=="templates_s"||flags.template_library);
    ...
    <div style={{marginLeft:220,flex:1,display:"flex",flexDirection:"column"}}>
      <header style={{height:68,display:"flex",alignItems:"center",padding:"0 28px",background:"#fff",borderBottom:`1px solid ${C.border}`,position:"fixed",top:0,left:220,right:0,zIndex:20}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <style>{`@keyframes fipilotPurpleGlow{0%,100%{box-shadow:0 0 0 0 rgba(124,58,237,0.35);}50%{box-shadow:0 0 0 8px rgba(124,58,237,0);}}@keyframes fipilotShake{0%,91%,100%{transform:translateX(0);}92%{transform:translateX(-2px);}93%{transform:translateX(2px);}94%{transform:translateX(-2px);}95%{transform:translateX(2px);}96%{transform:translateX(-1px);}97%{transform:translateX(1px);}98%,99%{transform:translateX(0);}}`}</style>
          {flags.ai_chat&&<button onClick={()=>setShowAICoach(true)} style={{padding:"7px 16px",borderRadius:10,background:"linear-gradient(135deg,#7C3AED,#5B21B6)",color:"#fff",fontSize:12,fontWeight:700,border:"none",cursor:"pointer",animation:"fipilotPurpleGlow 2s ease-in-out infinite, fipilotShake 4s ease-in-out infinite"}}>✨ Fipilot AI</button>}
      ...
      <main style={{padding:tab==="account"?"92px 100px 24px":"92px 24px 24px",flex:1}}>
        {tab==="dashboard"&&<div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"55fr 45fr",gap:24}}>
            <div style={{...card,padding:20}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><span style={{fontSize:15,fontWeight:800,color:C.t1}}>Danh sách thực đơn</span><span onClick={()=>setTab("meals")} style={{fontSize:12,color:C.primary,fontWeight:700,cursor:"pointer"}}>Xem tất cả →</span></div>
              {(() => {
                ...
                const eatenCount = pcWithItems.filter(m=>(eatenMeals||[]).includes(m.id)).length;
                const allEaten = pcWithItems.length > 0 && eatenCount === pcWithItems.length;
                const someEaten = eatenCount > 0 && !allEaten;
                const remaining = pcWithItems.length - eatenCount;
                const pcCalOk = pcAC >= pcHCal * 0.9;
                const pcCalShort = pcHCal - pcAC;
                return <>
                  ...
                  {someEaten && remaining > 1 && <div style={{padding:"7px 12px",borderRadius:8,fontSize:11,fontWeight:700,textAlign:"center",background:"#FEF3C7",color:"#92400E",border:"1px solid #FDE68A",marginBottom:6}}>🍽️ Đã ăn {eatenCount}/{pcWithItems.length} bữa — tiếp tục nhé!</div>}
                  ...
                  {allEaten && <div style={{textAlign:"center",padding:"20px 16px",background:"linear-gradient(135deg,#FFF7ED,#FEF3C7,#FDE68A)",borderRadius:16,border:"1.5px solid #F59E0B",marginBottom:6,position:"relative",overflow:"hidden"}}>
                    ...
                    <div style={{position:"absolute",top:8,right:12,fontSize:24,opacity:.6}}>🎊</div>
                    <div style={{fontSize:42,marginBottom:6,position:"relative"}}>🎉</div>
                    <div style={{fontSize:16,fontWeight:900,color:"#92400E",position:"relative"}}>Đã hoàn thành {eatenCount}/{pcWithItems.length} bữa ăn!</div>
                    <div style={{fontSize:12,color:"#B45309",marginTop:4,fontWeight:600,position:"relative"}}>{pcAC.toLocaleString()} / {pcHCal.toLocaleString()} kcal · {pcCalOk?"đạt mục tiêu":"còn thiếu "+Math.max(0,pcCalShort)+" kcal"}</div>
                    {pcCalOk&&<div style={{display:"inline-flex",alignItems:"center",gap:4,marginTop:8,padding:"4px 10px",background:"rgba(245,158,11,.15)",borderRadius:6,position:"relative"}}>
            ...
            <div>
              {(()=>{
                const pcWeightNow=new Date();
                const pcLastEntry=weightLog.length>0?weightLog[weightLog.length-1]:null;
                const pcLastLogDate=pcLastEntry?.logged_date||"";
                const pcDaysSince=pcLastLogDate?Math.floor((pcWeightNow.getTime()-new Date(pcLastLogDate+"T00:00:00").getTime())/(1000*60*60*24)):999;
                const pcWeighedRecently=pcDaysSince<7;
                const pcLastDay=pcLastEntry&&pcLastEntry.logged_date?new Date(pcLastEntry.logged_date+"T00:00:00").toLocaleDateString("vi-VN",{weekday:"short",day:"2-digit",month:"2-digit"}):(pcLastEntry?pcLastEntry.date:"—");
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  ...
                  {pcWeighedRecently
                    ? <div style={{display:"flex",alignItems:"center",gap:8}}>
                        ...
                        <span onClick={()=>setTab("weight")} style={{fontSize:11,color:"#007AFF",fontWeight:600,cursor:"pointer"}}>✏️ Sửa</span>
                      </div>
                ...
                {/* Input + banner chưa cân */}
                {pcShowWeightInput&&<div style={{background:C.surface,borderRadius:10,padding:"12px 14px",marginTop:12,border:`1.5px solid ${C.border}`}}>
                ...
                {!pcWeighedRecently&&!pcShowWeightInput&&<div onClick={()=>setPcShowWeightInput(true)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:"#FFF7ED",borderRadius:10,border:"1.5px solid #FDBA74",marginTop:12,cursor:"pointer"}}>
                  <span style={{fontSize:20}}>⚖️</span>
                  <div style={{flex:1}}>
                    <div style={{fontSize:12,fontWeight:700,color:"#9A3412"}}>Chưa cập nhật cân nặng tuần này</div>
                    <div style={{fontSize:10,color:"#C2410C",opacity:.7}}>Lần cuối: {pcLastDay} · {pcLastEntry?pcLastEntry.kg:"—"} kg</div>
                  </div>
          ...
          {pcAC>0&&pcCR>0&&(()=>{
            const pcWithItems=pcMeals.filter(m=>m.items&&m.items.length>0);
            const pcUneaten=eatenMeals&&eatenMeals.length>0?pcWithItems.filter(m=>!eatenMeals.includes(m.id)):[];
            if(pcUneaten.length>0){
              const names=pcUneaten.map(m=>m.name).join(", ");
              return <div style={{...card,padding:"18px 24px",marginTop:20,display:"flex",alignItems:"center",gap:16}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:C.t1}}>Fipilot AI gợi ý cho bạn</div><div style={{fontSize:12,color:C.t2,marginTop:3}}>Còn thiếu <b style={{color:"#D97706"}}>{pcCR} kcal</b>. Ăn nốt <b>{names}</b> để đạt mục tiêu hôm nay.</div></div>
              </div>;
            }
            return <div style={{...card,padding:"18px 24px",marginTop:20,display:"flex",alignItems:"center",gap:16}}>
              <div style={{width:48,height:48,borderRadius:14,background:"linear-gradient(135deg,#1E3A5F,#2D5A8E)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:24}}>🤖</div>
              <div style={{minWidth:0}}><div style={{fontSize:14,fontWeight:700,color:C.t1}}>Fipilot AI gợi ý cho bạn</div><div style={{fontSize:12,color:C.t2,marginTop:3}}>Hôm nay bạn đang thiếu <b style={{color:"#D97706"}}>{pcCR} kcal</b> để đạt mục tiêu.</div></div>
              <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
                <div style={{fontSize:12,color:C.t2,fontWeight:600,lineHeight:1.3}}>Gợi ý<br/>bổ sung:</div>
                <div style={{display:"flex",alignItems:"center",gap:4,padding:"10px 14px",background:C.surface,borderRadius:12}}><span style={{fontSize:22,marginRight:4}}>🍌</span><div><div style={{fontSize:12,fontWeight:700,color:C.t1}}>1 quả chuối</div><div style={{fontSize:10,color:C.t2}}>~ 27g tinh bột · 105 kcal</div></div></div>
                <span style={{color:"#CBD5E1",fontSize:16,fontWeight:300}}>+</span>
                <div style={{display:"flex",alignItems:"center",gap:4,padding:"10px 14px",background:C.surface,borderRadius:12}}><span style={{fontSize:22,marginRight:4}}>🍠</span><div><div style={{fontSize:12,fontWeight:700,color:C.t1}}>150g khoai lang</div><div style={{fontSize:10,color:C.t2}}>~ 28g tinh bột · 129 kcal</div></div></div>
        ...
        {tab==="diary_s"&&<div style={{maxWidth:800,margin:"0 auto",padding:"20px"}}><DiaryTab userId={user?.id} macro={macro}/></div>}
        ...
        {tab==="food_db_s"&&<AdminPanel key="fdb" {...adminP} forcedSection="settings" initialSection="food_db" hidePills/>}
        ...
        {tab==="backup_s"&&<AdminPanel key="backup" {...adminP} forcedSection="settings" initialSection="backup" hidePills/>}
        {tab==="ai_cost_s"&&<AdminPanel key="ai_cost" {...adminP} forcedSection="settings" initialSection="ai_cost" hidePills/>}
      </main>
    </div>
