...
export default function App(){
  ...
  if(mob) return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",background:C.bg,color:C.t1,minHeight:"100vh",padding:"0 10px 10px 10px",maxWidth:700,margin:"0 auto",overflowX:"hidden",width:"100%",boxSizing:"border-box"}}>
    ...
    <PWAInstallPrompt />
    {showPhotoMacro&&<PhotoMacroChecker onClose={()=>setShowPhotoMacro(false)} appSettings={appSettings} hasMealsToday={hasMealsToday} saveMealToCloud={saveMealToCloud} toggleEaten={toggleEaten} photoCtx={(()=>{try{const gd=profile?.gymDays||[0,2,4,5];const isNone=profile?.exerciseType==="none";const todayDow=new Date().getDay();const todayIsGym=!isNone&&gd.includes(todayDow);const activeDayType=todayIsGym?"train":"rest";const meals=getTodayMeals(activeDayType);const eatenCal=meals.filter(m=>eatenMeals.includes(m.id)).reduce((a,m)=>a+m.items.reduce((x,i)=>x+(i.cal||0),0),0);return{target:activeDayType==="train"?macro.calTarget:macro.calRest,eatenCal:Math.round(eatenCal),goal:macro.goal,proteinTarget:macro.protein,dayType:activeDayType};}catch(e){return null;}})()}/>}
