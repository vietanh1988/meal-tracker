    ...
    try{
      ...
      if(provider==="claude"){
        ...
        const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers,
          body:JSON.stringify({model:appSettings.ai_model||"claude-sonnet-4-20250514",max_tokens:500,messages:[{role:"user",content:prompt}]})});
      ...
      <button onClick={()=>{
        ...
        (async()=>{try{
          ...
          if(provider==="gpt"){
            ...
            const headers2={"Content-Type":"application/json","anthropic-version":"2023-06-01"};
            if(keys.claude)headers2["x-api-key"]=keys.claude;
            const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:headers2,
              body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:500,messages:[{role:"user",content:msg}]})});
            const d=await res.json();text=(d.content||[]).filter(b=>b.type==="text").map(b=>b.text).join("");
          }else{
  ...
  return <div>
    ...
    {section==="meals"&&<div style={card}>
      ...
      <div style={{borderTop:`1.5px solid ${C.border}`,paddingTop:14}}>
        <div style={{display:"grid",gridTemplateColumns:mob?"auto 1fr 56px 46px 64px 28px":"auto 2fr 60px 70px 80px 32px",gap:mob?4:8,marginBottom:8,alignItems:"center"}}>
        ...
        {foodItems.map((item,i)=>{
          ...
          return <div key={i} style={{display:"grid",gridTemplateColumns:mob?"auto 1fr 56px 46px 64px 28px":"auto 2fr 60px 70px 80px 32px",gap:mob?4:8,alignItems:"center",marginBottom:8}}>
          ...
          <select value={item.unit||"g"} onChange={onUnitChange} style={{...inp,textAlign:"center",padding:mob?"8px 0":"10px 4px",fontSize:16,appearance:"auto",WebkitAppearance:"auto"}}>
          ...
          <input type="number" inputMode="numeric" value={item.qty||""} onChange={e=>updateFood(i,"qty",Math.max(0,Number(e.target.value)||0))} style={{...inp,textAlign:"center",fontSize:16}} placeholder="SL"/>
          <input type="number" inputMode="numeric" value={isWeight?(item.gram||""):""} onChange={e=>{if(isWeight)updateFood(i,"gram",Math.max(0,Number(e.target.value)||0));}} readOnly={!isWeight} style={{...inp,textAlign:"center",fontSize:16,opacity:isWeight?1:0.35}} placeholder={isWeight?"Gram":"—"}/>
