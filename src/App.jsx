...
function Dashboard({weightLog,addWeight,profile,setProfile,macro,getMeals,appSettings,setTab,user,getWeeklyTemplate,applyTemplate}){if(!profile||!macro)return null;
  ...
  return <div>
    ...
    {meals.every(m=>!m.items||m.items.length===0)&&<div style={{...card,border:"2px dashed #CDCDCD",background:"transparent",textAlign:"center",padding:"20px 16px"}}>
      ...
      <div style={{fontSize:14,fontWeight:800,color:C.t2,marginBottom:4}}>Chưa có bữa ăn nào</div>
...
function AdminPanel({weightLog,setWeightLog,addWeight,deleteWeight,resetWeights,profile,setProfile,macro,saveMealToCloud,saveFoodCache,deleteFoodCache,getMeals,foodCache,appSettings,isAdmin,saveSetting,forcedSection,signOut,user,weeklyTemplates,saveWeeklyTemplate,getWeeklyTemplate,defaultTemplates,saveDefaultTemplate,deleteDefaultTemplate,applyTemplate}){if(!profile||!macro)return null;
  const mob=useIsMobile();
...
{"items":[{"name":"tên","gram":số,"protein":số,"carb":số,"fat":số,"fiber":số,"cal":số}],"tip":"1 câu gợi ý cho người gym"}`;
  ...
  return <div>
    ...
    {section==="admin"&&isAdmin&&<div style={card}>
      <div style={{fontSize:17,fontWeight:900,color:C.blue,marginBottom:4}}>🔧 Quản trị</div>
      <div style={{fontSize:13,fontWeight:600,color:C.t2,marginBottom:20}}>Quản lý thông báo và cập nhật cho tất cả users</div>

      <div style={{marginBottom:20}}>
        <div style={{fontSize:15,fontWeight:900,color:C.t1,marginBottom:12}}>📢 Quản lý thông báo</div>
      ...
      <div style={{borderTop:`2px solid ${C.border}`,paddingTop:16}}>
        <div style={{fontSize:15,fontWeight:900,color:C.t1,marginBottom:12}}>🔄 Force Update All Users</div>
        ...
        {step===4&&<div>
          ...
          <div style={{background:"linear-gradient(135deg,#111 0%,#2A0E0E 100%)",border:"2.5px solid #DC2626",borderRadius:14,padding:16,marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:14,marginTop:12}}>
              ...
              <MacroRing l="Carb" v={macro.carb} max={macro.carb} color="#EAB308" color2="#F59E0B" track="rgba(255,255,255,0.18)" tc="#FFF" unit="g"/>
              <MacroRing l="Fat" v={macro.fat} max={macro.fat} color="#8B5CF6" color2="#A78BFA" track="rgba(255,255,255,0.18)" tc="#FFF" unit="g"/>
