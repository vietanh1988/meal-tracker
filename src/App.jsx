  ...
  return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",display:"flex",minHeight:"100vh",background:C.bg,color:C.t1}}>
    ...
    <nav style={{width:220,background:"#fff",borderRight:`1px solid ${C.border}`,position:"fixed",top:0,left:0,bottom:0,zIndex:10,display:"flex",flexDirection:"column",padding:"20px 0",overflowY:"auto"}}>
      ...
      {isAdmin&&(()=>{
        const items=[
          ...
          {id:"ai_cost_s",l:"Chi phí AI",svg:(a)=><span style={{fontSize:17}}>💰</span>},
    ...
    {showAIMenuPC&&aiAccess.usable&&<div onClick={()=>setShowAIMenuPC(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:520,maxHeight:"88vh",overflowY:"auto",background:C.bg,borderRadius:16,padding:20}}>
        <AIMenuGenerator macro={macro} profile={profile} user={user} appSettings={appSettings} getMealHistory={getMealHistory} onApply={handleApplyAIMenuPC} onClose={()=>setShowAIMenuPC(false)}/>
      </div>
    </div>}
  </div>;
