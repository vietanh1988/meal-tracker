      ...
      {isAdmin&&(()=>{
        ...
        return <div style={{marginTop:16}}>
          <div onClick={()=>setAdminGrpOpen(p=>({...p,all:!p.all}))} style={{display:"flex",alignItems:"center",gap:8,padding:"0 20px 6px",cursor:"pointer",userSelect:"none"}}>
            <svg width={13} height={13} viewBox="0 0 24 24" style={{flexShrink:0}}><path d="M4 20 L4 6 C4 4.9 4.9 4 6 4 L10 4 L12 7 L18 7 C19.1 7 20 7.9 20 9 L20 18 C20 19.1 19.1 20 18 20 Z" fill="none" stroke="#EF4444" strokeWidth={2}/></svg>
            <span style={{fontSize:10,fontWeight:700,color:"#EF4444",letterSpacing:"0.8px",flex:1}}>QUẢN TRỊ</span>
          ...
          {open&&items.map(s=>{const a=tab===s.id;return <div key={s.id} onClick={()=>setTab(s.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 20px",cursor:"pointer",fontSize:14,fontWeight:a?700:500,color:a?C.primary:C.t2,background:a?"rgba(0,122,255,0.06)":"transparent",borderLeft:a?`3px solid ${C.primary}`:"3px solid transparent"}}>{s.svg(a)} {s.l}</div>;})}
        </div>;
      })()}
