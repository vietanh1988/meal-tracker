...
function ReportView({weightLog,profile,macro,getMealHistory,getDailyLogs,appSettings,mob}){
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
      <div style={{...card,padding:12,marginBottom:0}}><div style={{fontSize:11,fontWeight:700,color:C.t3,letterSpacing:"0.04em"}}>TB Calo/ngày</div><div style={{fontSize:22,fontWeight:700}}>{data.avgCal.toLocaleString()}</div><div style={{fontSize:11,marginTop:2,color:data.avgCal<data.target*0.9?"#EF4444":"#16A34A"}}>{data.avgCal<data.target*0.9?`Thiếu ${data.target-data.avgCal} cal`:"✓ Đạt mục tiêu"}</div></div>
  ...
  return <div>
    ...
    {!hidePills&&forcedSection==="settings"&&<div style={{display:"flex",borderBottom:`2px solid ${C.border}`,marginBottom:16,overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none",msOverflowStyle:"none"}}>
      {[{id:"account",t:"Tài khoản",svg:<svg viewBox="0 0 96 96" width={14} height={14}><defs><linearGradient id="pi5" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#40C8FF"/><stop offset="100%" stopColor="#0050FF"/></linearGradient></defs><rect x="4" y="16" width="88" height="64" rx="12" fill="url(#pi5)"/><circle cx="28" cy="42" r="16" fill="white" opacity="0.95"/><circle cx="28" cy="37" r="7" fill="url(#pi5)"/><path d="M14 54 C14 46 42 46 42 54" fill="url(#pi5)"/><rect x="52" y="34" width="32" height="7" rx="3.5" fill="white" opacity="0.9"/><rect x="52" y="46" width="24" height="6" rx="3" fill="white" opacity="0.5"/></svg>},
        ...
        {id:"weight",t:"Cân nặng",svg:<svg viewBox="0 0 96 96" width={14} height={14}><defs><linearGradient id="pi3" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#40C8FF"/><stop offset="100%" stopColor="#0050FF"/></linearGradient></defs><rect x="8" y="78" width="80" height="10" rx="5" fill="url(#pi3)"/><rect x="44" y="28" width="8" height="52" rx="4" fill="url(#pi3)"/><rect x="12" y="24" width="72" height="8" rx="4" fill="url(#pi3)"/><circle cx="48" cy="16" r="8" fill="url(#pi3)"/></svg>},
    ...
    {!hidePills&&forcedSection==="profile"&&<div style={{display:"flex",borderBottom:`2px solid ${C.border}`,marginBottom:16}}>
      {[{id:"profile",t:"Hồ sơ",svg:<svg viewBox="0 0 96 96" width={14} height={14}><defs><linearGradient id="pp1" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#40C8FF"/><stop offset="100%" stopColor="#0050FF"/></linearGradient></defs><circle cx="48" cy="30" r="24" fill="url(#pp1)"/><path d="M4 96 C4 60 92 60 92 96 Z" fill="url(#pp1)"/></svg>},
        {id:"schedule",t:"Lịch tập",svg:<svg viewBox="0 0 96 96" width={14} height={14}><defs><linearGradient id="pp2" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#40C8FF"/><stop offset="100%" stopColor="#0050FF"/></linearGradient></defs><rect x="10" y="18" width="76" height="68" rx="10" fill="url(#pp2)"/><rect x="10" y="18" width="76" height="26" rx="10" fill="url(#pp2)"/><rect x="10" y="34" width="76" height="10" fill="url(#pp2)"/><rect x="22" y="58" width="12" height="10" rx="3" fill="white" opacity="0.85"/><rect x="42" y="58" width="12" height="10" rx="3" fill="white" opacity="0.85"/><rect x="30" y="8" width="8" height="22" rx="4" fill="url(#pp2)"/><rect x="58" y="8" width="8" height="22" rx="4" fill="url(#pp2)"/></svg>}
      ].map(s=>
    ...
    {section==="meals"&&<div style={{...card,padding:mob?"12px 10px":"16px 18px"}}>
      {!mob?<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:4,background:C.surface,borderRadius:12,padding:4,justifyContent:"center"}}>
      ...
      {mealNames.map(meal=>{
        ...
        return <div key={meal.id} style={{background:C.card,border:`1.5px solid ${C.border}`,borderLeft:`3px solid ${mealColors[meal.id]||C.border}`,borderRadius:12,padding:mob?10:16,marginBottom:10}}>
          ...
          <button onClick={()=>{const u={...allFoodItems};const a=[...(u[meal.id]||[])];a.push({name:"",gram:"",unit:"g",qty:1});u[meal.id]=a;setAllFoodItems(u);}} style={{padding:"6px",fontSize:12,fontWeight:700,background:C.surface,color:C.t3,border:`1.5px dashed ${C.border}`,borderRadius:8,width:"100%",cursor:"pointer",fontFamily:"inherit",marginTop:4}}>+ Thêm món</button>
          {!mob&&aiResult&&(()=>{const items=aiResult.items||[];let idx2=0;for(const m2 of mealNames){const cnt=(allFoodItems[m2.id]||[]).filter(f=>f.name&&f.name.trim()).length;if(m2.id===meal.id){const sl=items.slice(idx2,idx2+cnt);const ms=sl.reduce((a,x)=>({p:a.p+(x.protein||0),c:a.c+(x.carb||0),f:a.f+(x.fat||0),fi:a.fi+(x.fiber||0),cal:a.cal+(x.cal||0)}),{p:0,c:0,f:0,fi:0,cal:0});if(sl.length>0)return <div style={{display:"flex",gap:14,marginTop:10,paddingTop:8,borderTop:`1px solid ${C.surface}`,flexWrap:"wrap"}}>
            <div style={{fontSize:12,color:C.t2,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.protein}}/> Protein: <b style={{color:C.t1}}>{Math.round(ms.p)}g</b></div>
            <div style={{fontSize:12,color:C.t2,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.carb}}/> Carb: <b style={{color:C.t1}}>{Math.round(ms.c)}g</b></div>
            <div style={{fontSize:12,color:C.t2,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.fat}}/> Fat: <b style={{color:C.t1}}>{Math.round(ms.f)}g</b></div>
            <div style={{fontSize:12,color:C.t2,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:C.fiber}}/> Chất xơ: <b style={{color:C.t1}}>{Math.round(ms.fi)}g</b></div>
            <div style={{fontSize:12,fontWeight:700,color:C.t1,marginLeft:"auto"}}>{Math.round(ms.cal)} kcal</div>
          </div>;return null;}idx2+=cnt;}return null;})()}
        </div>;
      })}
      <button onClick={()=>{
          ...
          {aiResult&&aiResult.items?(()=>{
            ...
            return <>
              <div style={{textAlign:"center",marginBottom:18,paddingBottom:16,borderBottom:`1px solid ${C.surface}`}}>
                ...
                <div style={{fontSize:14,color:C.t2}}>/ <b style={{color:C.t1}}>{heroCal}</b> kcal mục tiêu</div>
                <div style={{height:8,background:C.surface,borderRadius:4,overflow:"hidden",marginTop:10}}>
                  <div style={{height:"100%",borderRadius:4,width:`${Math.min(pct,100)}%`,background:`linear-gradient(90deg,#36A3FF,${pct>=95?"#34C759":"#007AFF"})`}}/>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
                  <span style={{fontSize:11,fontWeight:700,color:pct>=95&&pct<=110?"#34C759":pct>110?C.red:C.primary}}>{pct>=95&&pct<=110?"✓ Đạt mục tiêu":pct>110?"⚠ Vượt mục tiêu":`Còn thiếu ${heroCal-Math.round(s.cal)} kcal`}</span>
                  <span style={{fontSize:11,color:C.t2}}>{pct}%</span>
                </div>
              </div>
              {[{l:"Protein",v:Math.round(s.p),t:heroP,c:C.protein},{l:"Carb",v:Math.round(s.c),t:heroC,c:C.carb},{l:"Fat",v:Math.round(s.f),t:heroF,c:C.fat},{l:"Chất xơ",v:Math.round(s.fi),t:heroFi,c:C.fiber}].map(r=><div key={r.l} style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                <div style={{fontSize:13,fontWeight:600,width:70,display:"flex",alignItems:"center",gap:6}}><span style={{width:10,height:10,borderRadius:"50%",background:r.c,flexShrink:0}}/>{r.l}</div>
                <div style={{flex:1,height:6,background:C.surface,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",borderRadius:3,width:`${Math.min(r.t>0?r.v/r.t*100:0,100)}%`,background:r.c}}/></div>
                <div style={{fontSize:12,fontWeight:700,width:80,textAlign:"right"}}>{r.v}g <span style={{fontWeight:400,color:C.t2}}>/ {r.t}g</span></div>
              </div>)}
            </>;
          })():<div style={{textAlign:"center",padding:"40px 20px",color:C.t3}}>
            <div style={{fontSize:36,marginBottom:8}}>📊</div>
            <div style={{fontSize:14,fontWeight:600,color:C.t2}}>Chưa có dữ liệu</div>
            <div style={{fontSize:12,color:C.t3,marginTop:4}}>Nhấn "Tính macro tất cả" để xem kết quả</div>
          </div>}
        </div>
        {aiResult&&aiResult.items&&(()=>{
          const s=aiResult.items.reduce((a,i)=>({p:a.p+(i.protein||0),c:a.c+(i.carb||0),f:a.f+(i.fat||0),fi:a.fi+(i.fiber||0),cal:a.cal+(i.cal||0)}),{p:0,c:0,f:0,fi:0,cal:0});
          ...
          if(heroC>0){const r=s.c/heroC;scores.push(r>=0.9&&r<=1.1?100:Math.max(0,100-Math.round(Math.abs(1-r)*100)));}
          if(heroF>0){const r=s.f/heroF;scores.push(r>=0.85&&r<=1.15?100:Math.max(0,100-Math.round(Math.abs(1-r)*100)));}
          const avg=scores.length>0?Math.round(scores.reduce((a,b)=>a+b,0)/scores.length):0;
          return <div style={{background:C.card,border:`1.5px solid ${C.border}`,borderRadius:14,padding:20,marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
              <div style={{fontSize:14,fontWeight:800,display:"flex",alignItems:"center",gap:8}}>🎯 AI đánh giá</div>
              <div style={{fontSize:22,fontWeight:800,color:avg>=90?"#059669":avg>=70?C.primary:"#D97706"}}>{avg}<span style={{fontSize:13,fontWeight:500,color:C.t2}}>/100</span></div>
            </div>
            {[{l:"Calo",v:s.cal,t:heroCal,u:"kcal"},{l:"Protein",v:s.p,t:heroP,u:"g"},{l:"Carb",v:s.c,t:heroC,u:"g"},{l:"Fat",v:s.f,t:heroF,u:"g"},{l:"Chất xơ",v:s.fi,t:heroFi,u:"g"}].map(r=>{const pct2=r.t>0?Math.round(r.v/r.t*100):0;const ok=pct2>=90&&pct2<=115;return <div key={r.l} style={{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:12,padding:"6px 10px",background:C.surface,borderRadius:8,marginBottom:4}}>
              <span style={{color:C.t2}}>{r.l}</span>
              <span style={{fontWeight:700,color:ok?"#059669":"#D97706"}}>{ok?"✓":"⚠"} {pct2}%</span>
            </div>;})}
          </div>;
        })()}
        {aiResult&&aiResult.items&&<>
  ...
  return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",display:"flex",minHeight:"100vh",background:C.bg,color:C.t1}}>
    ...
    <nav style={{width:220,background:"#fff",borderRight:`1px solid ${C.border}`,position:"fixed",top:0,left:0,bottom:0,zIndex:10,display:"flex",flexDirection:"column",padding:"20px 0",overflowY:"auto"}}>
      ...
      {[{id:"ai",l:"Kết nối AI",svg:(a)=><svg viewBox="0 0 96 96" width={20} height={20}><defs><linearGradient id="si1" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="100%" stopColor={a?"#0050FF":"#64748B"}/></linearGradient></defs><rect x="28" y="28" width="40" height="40" rx="8" fill="url(#si1)"/><rect x="36" y="36" width="24" height="24" rx="4" fill="white" opacity="0.2"/><rect x="14" y="36" width="14" height="5" rx="2.5" fill="url(#si1)"/><rect x="14" y="46" width="14" height="5" rx="2.5" fill="url(#si1)"/><rect x="14" y="56" width="14" height="5" rx="2.5" fill="url(#si1)"/><rect x="68" y="36" width="14" height="5" rx="2.5" fill="url(#si1)"/><rect x="68" y="46" width="14" height="5" rx="2.5" fill="url(#si1)"/><rect x="68" y="56" width="14" height="5" rx="2.5" fill="url(#si1)"/><rect x="36" y="14" width="5" height="14" rx="2.5" fill="url(#si1)"/><rect x="46" y="14" width="5" height="14" rx="2.5" fill="url(#si1)"/><rect x="56" y="14" width="5" height="14" rx="2.5" fill="url(#si1)"/><rect x="36" y="68" width="5" height="14" rx="2.5" fill="url(#si1)"/><rect x="46" y="68" width="5" height="14" rx="2.5" fill="url(#si1)"/><rect x="56" y="68" width="5" height="14" rx="2.5" fill="url(#si1)"/></svg>},
        {id:"schedule",l:"Lịch tập",svg:(a)=><svg viewBox="0 0 96 96" width={20} height={20}><defs><linearGradient id="si2" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="100%" stopColor={a?"#0050FF":"#64748B"}/></linearGradient></defs><rect x="10" y="18" width="76" height="68" rx="10" fill="url(#si2)"/><rect x="10" y="18" width="76" height="26" rx="10" fill="url(#si2)"/><rect x="10" y="34" width="76" height="10" fill="url(#si2)"/><rect x="22" y="58" width="12" height="10" rx="3" fill="white" opacity="0.85"/><rect x="42" y="58" width="12" height="10" rx="3" fill="white" opacity="0.85"/><rect x="62" y="58" width="12" height="10" rx="3" fill="white" opacity="0.35"/><rect x="22" y="74" width="12" height="10" rx="3" fill="white" opacity="0.85"/><rect x="42" y="74" width="12" height="10" rx="3" fill="white" opacity="0.85"/><rect x="30" y="8" width="8" height="22" rx="4" fill="url(#si2)"/><rect x="58" y="8" width="8" height="22" rx="4" fill="url(#si2)"/></svg>},
        {id:"weight",l:"Cân nặng",svg:(a)=><svg viewBox="0 0 96 96" width={20} height={20}><defs><linearGradient id="si3" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="100%" stopColor={a?"#0050FF":"#64748B"}/></linearGradient></defs><rect x="8" y="78" width="80" height="10" rx="5" fill="url(#si3)"/><rect x="44" y="28" width="8" height="52" rx="4" fill="url(#si3)"/><rect x="12" y="24" width="72" height="8" rx="4" fill="url(#si3)"/><rect x="22" y="24" width="4" height="18" rx="2" fill="url(#si3)"/><rect x="70" y="24" width="4" height="18" rx="2" fill="url(#si3)"/><rect x="10" y="40" width="28" height="8" rx="4" fill="url(#si3)"/><rect x="58" y="40" width="28" height="8" rx="4" fill="url(#si3)"/><circle cx="48" cy="16" r="8" fill="url(#si3)"/></svg>},
      ...
      <main style={{padding:24,flex:1,overflow:"visible"}}>
        {tab==="dashboard"&&<div>
          ...
          <div style={{...card,padding:"28px 32px",borderRadius:20,display:"flex",alignItems:"center",marginBottom:24,border:`1.5px solid ${C.border}`}}>
            <div style={{flex:"0 0 40%"}}><div style={{fontSize:12,fontWeight:700,color:"#64748B",letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:8}}>{pcDayType==="train"?"Tổng calo ngày tập":"Tổng calo ngày nghỉ"}</div><div style={{fontSize:48,fontWeight:900,color:C.t1,letterSpacing:"-2px",lineHeight:1}}>{pcAC>0?pcAC.toLocaleString():pcHCal.toLocaleString()} <span style={{fontSize:17,fontWeight:600,color:"#64748B"}}> / {pcHCal.toLocaleString()} kcal</span></div>{pcAC>0&&<div style={{marginTop:10,fontSize:14,fontWeight:700,color:pcAC>=pcHCal*0.95&&pcAC<=pcHCal*1.1?"#34C759":"#F59E0B"}}>{pcAC>=pcHCal*0.95&&pcAC<=pcHCal*1.1?`✅ Đã đạt mục tiêu! ${pcAC-pcHCal} kcal`:`⚠️ Còn thiếu ${pcCR} kcal`}</div>}<div style={{display:"flex",alignItems:"center",gap:10,marginTop:14,maxWidth:320}}><div style={{flex:1,height:10,background:C.border,borderRadius:5}}><div style={{height:10,background:"linear-gradient(90deg,#36A3FF,#007AFF)",borderRadius:5,width:`${Math.min(pcAC>0?(pcAC/pcHCal)*100:0,120)}%`,transition:"width 0.4s"}}/></div><span style={{fontSize:14,fontWeight:700,color:C.primary}}>{pcAC>0?Math.round(pcAC/pcHCal*100):0}%</span></div></div>
