  ...
  if(mob) return <div style={{fontFamily:"'Inter',Roboto,-apple-system,'Segoe UI',sans-serif",background:C.bg,color:C.t1,minHeight:"100vh",padding:"0 10px 10px 10px",maxWidth:700,margin:"0 auto",overflowX:"hidden",width:"100%",boxSizing:"border-box"}}>
    ...
    {!isAdmin&&showFeedbackPrompt&&<div style={{position:"fixed",inset:0,zIndex:99999,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>{setShowFeedbackPrompt(false);localStorage.setItem("feedback_prompted","1");}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:20,padding:"28px 24px",maxWidth:340,width:"100%",textAlign:"center",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        <button onClick={()=>{setShowFeedbackPrompt(false);localStorage.setItem("feedback_prompted","1");setTab("settings");}} style={{width:"100%",padding:14,borderRadius:12,border:"none",fontSize:15,fontWeight:800,color:"#fff",background:"linear-gradient(135deg,#36A3FF,#007AFF)",cursor:"pointer",fontFamily:"inherit",marginBottom:8}}>💡 Góp ý ngay</button>
    ...
    <nav style={{width:220,background:"#fff",borderRight:`1px solid ${C.border}`,position:"fixed",top:0,left:0,bottom:0,zIndex:10,display:"flex",flexDirection:"column",padding:"20px 0",overflowY:"auto"}}>
      ...
      {isAdmin&&(()=>{
        const items=[
          ...
          {id:"food_db_s",l:"Kho thực phẩm",svg:(a)=><svg viewBox="0 0 96 96" width={17} height={17}><defs><linearGradient id="qi15" x1="0" y1="0" x2="96" y2="96" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor={a?"#40C8FF":"#64748B"}/><stop offset="55%" stopColor={a?"#0050FF":"#64748B"}/><stop offset="100%" stopColor={a?"#DC2626":"#64748B"}/></linearGradient></defs><ellipse cx="48" cy="24" rx="36" ry="14" fill="url(#qi15)"/><path d="M12 24 v20 c0 8 16 14 36 14 s36-6 36-14 v-20" fill="none" stroke="url(#qi15)" strokeWidth="4"/><path d="M12 44 v20 c0 8 16 14 36 14 s36-6 36-14 v-20" fill="none" stroke="url(#qi15)" strokeWidth="4" opacity="0.6"/></svg>},
